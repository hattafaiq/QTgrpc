...
class MathTestClient {
    public:
        ...
        //alurnya terima float array dulu dalam byte
        //atau terima float yang sudah jadi char ke byte;
        int sendSofHaliza(int a, int b) {
            RecBro request;
            Datablob respon;

            request.set_header("minta data");
            std::cout << " -------> " << request.header() << " <------- \n"<< std::endl;
            ClientContext context;

            Status status = stub_->Softhaliza(&context, request, &respon);

            if(status.ok()){
                if(respon.header()=="kirim data"){
                    std::cout << " -------> " << respon.header() << " <------- \n"<< std::endl;
                    std::cout << "Tampung tampung tampung \n" << std::endl;
                }
                if(respon.header()=="selesai"){
                    std::cout << " -------> " << respon.header() << " <------- \n"<< std::endl;
                    std::cout << "sudah selesai minta data \n" << std::endl;
                }
            }
            else {
        ...
        int sendRequest(int a, int b) {
            ...
            request.set_a(2);
            request.set_b(4);
            //std::cout << " -------> a:" << request.a()<<" b:"<< request.set_b()<< " <------- \n"<< std::endl;
            ClientContext context;

            Status status = stub_->SendRequest(&context, request, &respon);
            if(status.ok()){
